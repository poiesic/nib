...
type ExtractFunc func(prompt string, schema json.RawMessage, dir string, effort agent.Effort) (json.RawMessage, error)
...
type IndexOptions struct {
	...
	Effort    agent.Effort // effort level for extraction
}
...
func extractScene(db *storydb.DB, slug string, isInterlude bool, projectRoot string, book *binder.Book, extractFn ExtractFunc, opts IndexOptions, characterSlugs []string, earlierExtractions []*ExtractionResult, statusFn func(string)) (*ExtractionResult, bool, error) {
	...
	data, err := extractFn(prompt, json.RawMessage(jsonSchema), projectRoot, opts.Effort)
	if err != nil {
